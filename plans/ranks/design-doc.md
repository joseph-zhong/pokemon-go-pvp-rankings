...
## 17. Top 5 & nearby ranks return, adapted to per-league cards; the slider skips Master-best stages

**Top 5 & nearby ranks.** §10's original design assumed a single query result (one species, one league). The page has since grown to show all 3 leagues per evolution stage at once (§15's redesign), so there's no single "the" result to attach a table to anymore — same problem "Recommended moves" already solved. `renderRankTables(entry, league, ivs)` reuses that exact solution: it renders for whichever (stage, league) pair the moves section is already showing (`bestEntry`/`bestLeague` from `render()`, respecting the moves-league toggle), off the same `rankingsByStage` array `rankAllIvs` already built — no new computation, just two more views onto data that already existed. Top 5 is `all.slice(0, 5)`; Nearby is an 11-wide window centered on the query's rank, clipped at the #1/#4096 edges, matching §10 exactly. Both render as small clickable tables (`src/ranks/main.ts`: `fillRankTable`) — clicking a row sets the IV steppers to that combo's IVs and re-renders, the same as typing them.

**Slider hidden for a stage whose own best league is Master.** Master League has no CP cap, so §16's "where does evolving/leveling cross the cap" question has no answer worth showing for a Pokemon that's actually a Master League pick (e.g. Mewtwo: rank #1 in Master League, but only a mediocre rank in Great/Ultra it was never meant to be played at — the slider would just show where you'd have to stunt it down to roughly level 12 to force-fit a cap nobody's targeting). Each stage now compares its own Great/Ultra/Master ranks (not the evolve-target's — a pre-evolution that's independently Great-League-viable, like Dragonair, still needs its slider even though its evolution Dragonite is Master-tier) and hides the whole slider (`card.slider.wrap.hidden`) when Master wins. Cost: three extra `findCombo` lookups per stage per render — negligible against the 4096-entry arrays already being scanned.

**Sliders sync to one shared level.** Each stage's slider used the same MIN_LEVEL and the same level cap (Best Buddy applies line-wide), so every card's track already represented the identical axis — but dragging one didn't move the others, so comparing "at my actual level, which stages in this family are still cap-eligible" meant reading marks off separate cards independently and doing the comparison in your head. Since level carries over through evolution in the real game (there's no such thing as "Chansey is level 30 but Blissey would be level 25"), one dragged level should mean the same thing everywhere at once. `buildLevelSlider`'s input handler now calls `syncLevel(level)`, which writes that value into every stage's `<input type="range">` and refreshes every readout (`src/ranks/main.ts`). Only the thumb position and the "Level N · CP M" text move — the Great/Ultra marks themselves don't, since they mark a fixed crossing point independent of wherever the thumb currently sits. This is what makes the `→ Great`/`→ Ultra` marks from §16 actually useful together rather than as N separate facts: drag to any level once, and every stage's card shows whether it's still under each cap at that exact level, side by side.

## 18. One rank table, not two

§17 shipped Top 5 and Nearby as two side-by-side tables. For a query anywhere near the top that reads as a bug: both tables show #1-#5, the same five rows twice, and neither says how far apart the two blocks actually are. The ranks are one ordered list, so they belong in one table.

`rankTableRows(all, targetRank, topCount = 5, radius = 5)` (`src/calc/rank.ts`) builds the row list: the top 5, then the ±5 window around the query. When the window starts at or before rank 6 the two ranges touch, so it returns a single contiguous slice and nothing is repeated. Otherwise it splices in one `{ kind: "gap", skipped }` row between them, rendered as `⋯ N ranks hidden` across all five columns — the reader sees the distance instead of inferring it from a jump in the rank column. Keeping this as a pure function (rather than inline in the renderer) is what makes the collapse/gap boundary cases unit-testable; `fillRankTable` just walks the rows. The card's note line also now carries the query's own position (`rank #195 of 4,096`), which the two-table version left the reader to find in the highlighted row.
